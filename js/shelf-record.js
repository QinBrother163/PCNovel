...
!function () {
    ...
    var J_LB = document.getElementById('J_LB');
    var J_LB_list = J_LB.querySelectorAll('.J_LB_list');
    var J_LB_J_LB_DT = document.getElementById('J_LB_DT');
    var J_LB_control = J_LB_J_LB_DT.querySelectorAll('.control');
    var J_LBm_booklist1 = J_LB.querySelector('.m-booklist-1');

    var J_YueduSNSLB = J_LB.getElementsByClassName('J_YueduSNS');


    var m_s1 = J_LB_J_LB_DT.querySelectorAll('.m_s1');

    var LB_DT_f_db2 = J_LB_J_LB_DT.querySelectorAll('.f-db2');
    var LB_DT_m_infos = J_LB_J_LB_DT.querySelectorAll('.m-infos');
    var LB_DT_m_bookshow = J_LB_J_LB_DT.querySelectorAll('.m-bookshow');
    ...
    var controlList1 = J_LB_J_LB_DT.querySelectorAll('.control');
    ...
    var J_LB_tick = J_LB_J_LB_DT.querySelectorAll('.tick');//111111111111
    ...
    //列表删除操作列表
    var LB_selectall = J_LB.querySelectorAll('.selectall');
    var LB_ipt = J_LB.querySelectorAll('.LB_ipt');
    var LB_Jtoolbar1 = J_LB.querySelector('.J-toolbar-1');
    var LB_Jtoolbar2 = J_LB.querySelector('.J-toolbar-2');
    var LB_body = J_LB.querySelector('#LB_body');


    //列表加入书架
    function addListLB() {
        for (var i = 0; i < m_s1.length; i++) {
            m_s1[i].idx = i;
            m_s1[i].onclick = addListClickLB;
        }
    }

    addListLB();
    function addListClickLB() {
        showStatus('下标 :' + this.idx + '加入书架成功！')
    }
    ...
    function setLBDel(nodeAll, nodeList, J1, J2, body, nodeWrap) {
        ...
        J2.querySelector('#J_DelSelectDT').onclick = function () {
            ...
            // } else {
            //     J_CBinner2Link[1].innerText = '已删书籍(' + liLen.length + ')';
            //     linkA[1].innerText = '已删书籍(' + liLen.length + ')';
            // }
    ...
    function initList() {
        ...
        setLBDel(LB_selectall, LB_ipt, LB_Jtoolbar1, LB_Jtoolbar2, LB_body, J_LB);
    ...
    function setClickControl() {
        ...
        open2 = J_LB_tick[this.idx].className == 'tick j-mask tick-x';

        J_LB_tick[this.idx].className = open2 ? 'tick j-mask tick-o' : 'tick j-mask tick-x';
    ...
    J2btn_del.onclick = function () {
        ...
        if (delNode.length == 0) {
            ...
            if (nodeLi.length == 0) {
                ...
                controlList1 = J_LB_J_LB_DT.querySelectorAll('.control');

                // linkA[1].innerText = '已删书籍(' + controlList1.length + ')';
                //
                // J_CBinner2Link[1].innerText = '已删书籍(' + controlList1.length + ')';
            }
            idx_num = 0;
        }
    };
    ...
    btn_finish.onclick = function () {
        ...
        showControlList(controlList1, 'none');
        setAllBoole(J_LB_tick, 'tick j-mask tick-x');
    };
    ...
    J_Settings.onclick = function () {
        ...
        setControl(J_LB_control);

        showControlList(controlList1, 'block');
    };
    ...
    function setShowList(idx) {
        ...
        if (m_shelfempty.style.display != 'block'){
            for (var i = 0; i < J_LB_list.length; i++) {
                J_LB_list[i].style.display = 'none';
            }
            J_LB_list[idx].style.display = 'block';
        }
    ...
    function setListStatus() {
        ...
        function Q(idx) {
            ...
            if (idx == 0) {
                J_LBm_booklist1.style.display = 'block';
            } else {
                J_LB_J_LB_DT.style.display = 'block';
            }
        }
    ...
    function setDBMouseoverLB() {
        for (var i = 0; i < LB_DT_f_db2.length; i++) {
            LB_DT_f_db2[i].onmouseover = LBMouseover;
            LB_DT_f_db2[i].onmouseout = LBMouseout;
            LB_DT_f_db2[i].idx = i;
        }
    }
    ...
    function LBMouseover() {
        for (var i = 0; i < LB_DT_f_db2.length; i++) {
            LB_DT_m_infos[i].style.display = 'none';
            LB_DT_m_bookshow[i].style.zIndex = '0';
        }
        if (LB_DT_f_db2[this.idx] == this) {
            LB_DT_m_infos[this.idx].style.display = 'block';
            LB_DT_m_bookshow[this.idx].style.zIndex = '1000';
        }
    }
    ...
    function setNodeLB() {
        for (var i = 0; i < J_YueduSNSLB.length; i++) {
            J_YueduSNSLB[i].onclick = setClickLB;
        }
    }

    setNodeLB();

    function setClickLB(event) {
